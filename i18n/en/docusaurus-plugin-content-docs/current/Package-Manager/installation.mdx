...
The RuyiSDK package manager is now [available on PyPI](https://pypi.org/project/ruyi/)，and can be installed using `pip` or other Python package managers.
...
RISC-V users can install `ruyi` via `pip`, but please note that some Python libraries required by `ruyi` do not yet offer precompiled packages for the RISC-V architecture on PyPI. As a result, the Python package manager may attempt to compile these dependencies from source during installation, which can be time-consuming or may fail.

If you encounter issues installing `ruyi` on a RISC-V device, we recommend using alternative installation methods.
