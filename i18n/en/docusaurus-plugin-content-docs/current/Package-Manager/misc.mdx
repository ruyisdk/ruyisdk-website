...
You can choose whether to enable telemetry data collection by setting the `RUYI_TELEMETRY_OPTOUT` environment variable. Any of `1`, `true`, `x`, `y`, or `yes` (case-insensitive) disables telemetry and is equivalent to the `off` mode below.
...
Starting from `ruyi` 0.42.0 (inclusive), the default telemetry mode is `local`, which means each `ruyi` invocation records some non-sensitive operational data and some state information locally, but nothing is uploaded automatically. Uploads only happen when you explicitly enable them (for example, by switching to the `on` mode or running `ruyi telemetry upload`).

When the telemetry mode is `on`, collected data is uploaded weekly to servers managed by the RuyiSDK team in the People's Republic of China. The upload day is determined solely by the installation's anonymous ID.
...
You can view our [privacy policy](https://ruyisdk.org/en/docs/legal/privacyPolicy/) on the RuyiSDK official website.
