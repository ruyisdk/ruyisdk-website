...
1. Ensure that you have downloaded and installed `ruyi`. Please refer to the [RuyiSDK Package Manager Installation Guide](https://ruyisdk.org/en/docs/Package-Manager/installation/) for installation instructions (it is recommended to download the latest version). After installation, you can execute `ruyi --version` to check the version information.
...
The K230D SDK provides [build instructions](https://github.com/ruyisdk/k230_linux_sdk/blob/dev/README.md) for the image. After the build is completed, the image files are stored in the `output` directory. Depending on the defconfig executed, the image files for 64ILP32 and 64LP64 are stored in the following paths:
