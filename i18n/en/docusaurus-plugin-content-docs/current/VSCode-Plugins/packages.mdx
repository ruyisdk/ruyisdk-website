...
Note: Package sources, naming, and versioning follow the same conventions as the CLI. For detailed functionality, see [Ruyi Package Manager](../Package-Manager/index.md).
...
If network access is restricted, configure a mirror source for Ruyi in the terminal first, then return to the extension for installation and updates. For CLI configuration methods, see [installation](../Package-Manager/installation.mdx).
