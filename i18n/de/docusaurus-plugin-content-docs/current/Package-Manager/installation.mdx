...
import ReleaseProvider, { DownloadRuyi, ChmodCommand, CpCommand } from '@site/src/components/Docs/LatestReleases';
...
Wir empfehlen die Installation mit dem Skript. Es erkennt Ihre Architektur automatisch, lädt zuerst die neueste stabile Version vom ISCAS-Mirror herunter und installiert sie nach `~/.local/bin/ruyi`:
...
Sie können die vorgefertigte Binärdatei auch manuell herunterladen.
...
Um den manuellen Download zu beschreiben, verwenden wir das ISCAS-Mirror-Source-Beispiel. Führen Sie beim manuellen Download zunächst ``uname -m`` aus, um die Systemarchitektur zu überprüfen und die entsprechende Binärdatei herunterzuladen.
...
Das Standardinstallationsverzeichnis ist ``~/.local/bin``. Wenn ``ruyi`` nach der Installation nicht gefunden wird, stellen Sie sicher, dass ``~/.local/bin`` in Ihrem ``PATH`` enthalten ist, und melden Sie sich bei Bedarf erneut an oder starten Sie Ihre Shell neu.
