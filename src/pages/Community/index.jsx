  ...
  <div className={`${styles.avatarWrapper} ${sizeClass}`}>
    ...
    {githubUrl && (
      <a
        ...
        className={styles.githubIconOnAvatar}
      >
  ...
  return (
    <div className={styles.coreMemberCard}>
      <div className={styles.coreMemberLeftColumn}>
        ...
        {title && (
          <div className={styles.personTitle} style={titleStyle}>
            {title}
          </div>
        )}
      </div>
  ...
  return (
    <div className={styles.internCard}>
      <AvatarWithGithub
        ...
        name={name}
        githubUrl={github}
        sizeClass={styles.avatarMedium}
      />
      <div className={styles.internInfo}>
        <h4 className={styles.personNameSmall}>{name}</h4>
        {title && (
          <div className={styles.personTitleSmall} style={titleStyle}>
            {title}
          </div>
        )}
        {description && (
          <p className={styles.personDescriptionSmall}>{description}</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className={`${styles.glassContainer} ${styles.communityIntro}`}>
      <div className={styles.communityIntroLeft}>
        <BackgroundBlobs />
        <h2 className={styles.communityIntroTitle}>
          <Translate>贡献者社区</Translate>
        </h2>
      </div>
      <div className={styles.communityIntroRight}>
        <p className={styles.communityIntroText}>
          <Translate>
            RuyiSDK 项目采用开源管理模式，代码托管于
            GitHub。我们欢迎广大开发者参与项目，共同推动RISC-V技术的发展。RUYISDK
            社区旨在建设一个开放、友善、多样化、包容、健康活跃的社区。我们正在寻找有激情、有才华的您加入我们的团队。在参与社区讨论前，请先查阅社区守则以便更好的在社区内交流。
          </Translate>
        </p>
        <a
          ...
          <div className={styles.partnerLogoContainer}>
            <img
              ...
              onError={(e) => {
                e.target.onerror = null;
                e.target.src =
                  "https://placehold.co/200x80/ffffff/000000?text=" +
                  partner.name;
              }}
            />
          </div>
...
const Text = ({ markdownContent }) => {
  const htmlContent = markdownContent
    ...
    .map((line) => {
      if (line.startsWith("## "))
        return `<h2 class="text-2xl font-bold mt-4 mb-2" style="color: #2D3748; font-size: 1.5rem; margin-top: 1rem; margin-bottom: 0.5rem;">${line.substring(3)}</h2>`;
      if (line.startsWith("# "))
        return `<h1 class="text-3xl font-bold mt-6 mb-3" style="color: #1A202C; font-size: 1.875rem; margin-top: 1.5rem; margin-bottom: 0.75rem;">${line.substring(2)}</h1>`;
  ...
  return (
    <div
      className={`${styles.glassContainer} ${styles.textContainer}`}
...
export default function Community() {
  ...
  const partnersData = [
    {
      id: 1,
      name: "Milk-V",
      logoUrl: "https://www.fedoravforce.org/partner-logo/milkv.png",
      url: "https://milkv.io/",
    },
    {
      id: 2,
      name: "Sipeed",
      logoUrl: "https://www.fedoravforce.org/partner-logo/sipeed.png",
      url: "https://sipeed.com/",
    },
    {
      id: 3,
      name: "Fedora-V Force",
      logoUrl: "https://images.fedoravforce.org/images/fvf-logo.png",
      url: "https://www.fedoravforce.org/",
    },
    {
      id: 4,
      name: "openEuler RISC-V sig",
      logoUrl: "https://www.fedoravforce.org/partner-logo/openeuler.png",
      url: "https://www.openeuler.org/en/sig/sig-detail/?name=sig-RISC-V",
    },
    {
      id: 5,
      name: "openKylin",
      logoUrl: "https://www.openkylin.top/upload/202209/1664440595.png",
  ...
  useEffect(() => {
    ...
    fetch(mdPublicUrl)
      ...
      .catch((error) => {
        ...
        setCommunityGuidelines(
          `Error loading guidelines: ${error.message}. Please check console.`,
        );
      });
    ...
    try {
      ...
      console.error(
        "Error loading peoples JSON file for locale:",
        detectedLocale,
        error,
      );
  ...
  return (
    <Layout
      title="Community"
      description="Meet the RuyiSDK community, contributors, and partners."
    >
      <PageBackground />
      <div className={styles.pageWrapper}>
        <div className={styles.contentContainer}>
          ...
          <h2 className={styles.sectionHeader}>
            <Translate>核心小组</Translate>
          </h2>
          <div className={`${styles.glassContainer} ${styles.coreGrid}`}>
            {peopleData.coreTeam.map((person) => (
              <CoreMemberCard key={person.id} person={person} />
            ))}
          </div>

          <h2 className={styles.sectionHeader}>
            <Translate>我们❤️实习生</Translate>
          </h2>
          <div className={`${styles.glassContainer} ${styles.internGrid}`}>
            {peopleData.interns.map((person) => (
              <InternCard key={person.id} person={person} />
            ))}
          </div>

          <h2 className={styles.sectionHeader}>
            <Translate>贡献者</Translate>
          </h2>
          <div className={`${styles.glassContainer} ${styles.contributorGrid}`}>
            {peopleData.contributors.map((person) => (
              <ContributorCard key={person.id} person={person} />
            ))}
          </div>
          ...
          <h2 className={styles.sectionHeader}>
            <Translate>贡献者公约</Translate>
          </h2>
          {communityGuidelines ? (
            <Text markdownContent={communityGuidelines} />
          ) : (
            <p>Loading community guidelines...</p>
          )}
        </div>
      </div>
    </Layout>
