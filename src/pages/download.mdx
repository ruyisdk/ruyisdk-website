import ReleaseProvider, { DownloadRuyi, useReleaseData } from '@site/src/pages/LatestReleases';
...
{(() => {
  ...
  function ArchControls() {
    ...
    const [arch, setArch] = useState(detectArch());
    ...
    useEffect(() => {
      // Update arch if user agent indicates a clearer architecture after hydration
      setArch(prev => prev); // no-op to satisfy lint; placeholder for future signals
    }, []);
