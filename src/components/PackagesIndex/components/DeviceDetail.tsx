...
export default function DeviceDetail() {
  ...
  const variants = Object.keys(hierarchy).filter(
    (key) =>
      key.startsWith("device-variant:") && hierarchy[key].includes(deviceId),
  );

  // Find CPUs for those variants
  const cpus = Object.keys(hierarchy).filter(
    (key) =>
      key.startsWith("cpu:") &&
      variants.some((v) => hierarchy[key].includes(v)),
  );

  // Find uarches for those CPUs
  const uarches = Object.keys(hierarchy).filter(
    (key) =>
      key.startsWith("uarch:") && cpus.some((c) => hierarchy[key].includes(c)),
  );

  // Find arches for those uarches
  const arches = Object.keys(hierarchy).filter(
    (key) =>
      key.startsWith("arch:") &&
      uarches.some((u) => hierarchy[key].includes(u)),
  );
  ...
  return (
    <div
      className="pi-animate-fade-in-up flex min-h-screen flex-col
        text-[var(--text)]"
    >
      <main
        className="mx-auto w-full max-w-[90rem] flex-1 px-4 py-8 sm:px-6
          lg:px-8"
      >
          ...
          <div className="space-y-6 md:col-span-1">
            <div className="mb-2 flex flex-wrap items-center gap-3">
              <button
                type="button"
                onClick={handleBack}
                className="pi-button-secondary inline-flex h-10 items-center
              ...
              <h1
                className="m-0 flex min-h-10 items-center text-xl leading-none
                  font-bold text-[var(--ifm-color-primary)]"
              >
                {displayName}
              </h1>
            </div>
            <section className="pi-surface rounded-[1.5rem] p-6">
              ...
              <dl className="space-y-4 text-sm">
                ...
                {arches.length > 0 && (
                  <div>
                    <dt className="flex items-center gap-1 text-[var(--subtle)]">
                      <Layers className="h-3 w-3" /> {t("architectures")}
                    </dt>
                    <dd className="mt-1 flex flex-wrap gap-2">
                      {arches.map((a) => (
                        <span
                          key={a}
                          className="rounded-md bg-[var(--tintColor)] px-2 py-1
                            font-mono text-xs text-[var(--subtle)]"
                        >
                          {a.replace("arch:", "")}
                        </span>
                      ))}
                      ...
                      {variants.map((v) => (
                        <span
                          ...
                          className="rounded-md bg-[var(--tintColor)] px-2 py-1
                            font-mono text-xs text-[var(--subtle)]"
                        >
                          {variantNames.get(v.replace("device-variant:", "")) ||
          ...
          <div className="space-y-6 md:col-span-2">
            <section>
              <h2 className="mb-4 flex items-center gap-2 text-xl font-semibold">
                ...
                {t("supportedPackages")}
                <span
                  className="rounded-full bg-[var(--tintColor)] px-2 py-0.5
              ...
              {supportedPackages.length > 0 ? (
                  ...
                  {supportedPackages.map((pkg) => (
                    ...
                    >
                      <article className="pi-card h-full p-4">
                        ...
                        <div className="mt-3">
                          <span
                            className="pi-chip px-2 py-0.5 text-[10px]
                              font-medium"
                          >
                            {pkg.category}
                          </span>
                ...
                <div
                  className="rounded-[1.5rem] border border-dashed
                    border-[var(--divider)] bg-white p-8 text-center text-sm
