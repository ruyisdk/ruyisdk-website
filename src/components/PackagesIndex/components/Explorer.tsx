...
export default function Explorer() {
  ...
  const [activeTab, setActiveTab] = useState<"devices" | "packages">(
    lastActiveTab,
  );
  const [searchQuery, setSearchQuery] = useState(lastSearchQuery);
  const [selectedArchs, setSelectedArchs] =
    useState<Set<string>>(lastSelectedArchs);
  const [selectedCpus, setSelectedCpus] =
    useState<Set<string>>(lastSelectedCpus);
  ...
  const renderIosSelector = () => (
    <div
      className="relative mb-4 flex w-full rounded-lg bg-[#e3e3e6] p-0.5
        dark:bg-zinc-800"
    >
      <div
        className={`absolute top-0.5 bottom-0.5 left-0.5 w-[calc(50%-2px)]
          rounded-md bg-white shadow-sm transition-transform duration-200
          ease-[cubic-bezier(0.16,1,0.3,1)] dark:bg-zinc-700 ${
      ...
      <button
        ...
        className={`z-10 flex flex-1 items-center justify-center gap-1
          rounded-md py-1.5 text-xs font-semibold transition-colors duration-200
          ${
            activeTab === "devices"
      ...
      >
        {t("listByDevice")}
        <span
          className={`py-0.2 rounded-full px-1.5 text-[10px] transition-colors
            duration-200 ${
              activeTab === "devices"
                ? `bg-zinc-100 text-zinc-600 dark:bg-zinc-800
                  dark:text-zinc-300`
                : `bg-zinc-200/50 text-zinc-500 dark:bg-zinc-700/50
                  dark:text-zinc-400`
            }`}
        >
          {devices.length}
  ...
  const filteredDevices = useMemo(() => {
    ...
    return devices.filter((device) => {
      if (validDeviceIds && !validDeviceIds.has(`device:${device.id}`))
        return false;
  ...
  const searchPlaceholder =
    activeTab === "devices"
      ? t("searchPlaceholderDevices")
      : t("searchPlaceholderPackages");
  const showingCount =
    activeTab === "devices" ? filteredDevices.length : filteredPackages.length;
  const listLabel =
    activeTab === "devices" ? t("listByDevice") : t("listByPackages");
  const activeDescription =
    activeTab === "devices"
      ? t("deviceListDescription")
      : t("packageListDescription");

  return (
    <div
      className="pi-animate-fade-in-up flex h-full flex-col text-[var(--text)]"
    >
      {mobileFiltersOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          />
          <aside
            className="pi-surface absolute top-0 right-0 h-full w-80 bg-white
              p-4 shadow-lg"
          >
              ...
              <label className="relative block">
                ...
                <Search
                  className="pointer-events-none absolute top-1/2 left-3 h-4 w-4
                    -translate-y-1/2 text-[var(--subtle)]"
                />
                <input
                  type="search"
                  value={searchQuery}
                  ...
                  className="w-full rounded-full border border-[var(--divider)]
                    bg-white py-2 pr-3 pl-9 text-sm transition outline-none
                    focus:border-[var(--ifm-color-primary)] focus:ring-2
                    focus:ring-[var(--ifm-color-primary)]/15"
                />
              </label>
            </div>
            {renderIosSelector()}
            {activeTab === "devices" ? (
                ...
                <section>
                  <h2
                    className="mb-3 flex items-center gap-2 text-sm
                      font-semibold"
                  >
                    <Layers className="h-4 w-4 text-[var(--subtle)]" />{" "}
                    {t("architectures")}
                  </h2>
                    ...
                    {archs.map((arch) => (
                      <label
                        ...
                        className="flex cursor-pointer items-center gap-2
                          text-sm text-[var(--light)] transition
                          hover:text-[var(--ifm-color-primary)]"
                      >
                        <input
                          type="checkbox"
                          ...
                          className="rounded border-[var(--divider)]
                            ...
                            focus:ring-[var(--ifm-color-primary)]"
                        />
                    ...
                    {cpus.map((cpu) => (
                      ...
                      >
                        <input
                          ...
                          onChange={() =>
                            toggleFilter(selectedCpus, cpu.id, setSelectedCpus)
                          }
                          className="rounded border-[var(--divider)]
                            text-[var(--ifm-color-primary)]
                            focus:ring-[var(--ifm-color-primary)]"
              ...
              <section>
                ...
                >
                  <Tag className="h-4 w-4 text-[var(--subtle)]" />{" "}
                  {t("categories")}
                </h2>
      ...
      <div
        className="mx-auto flex w-full max-w-[90rem] flex-1 overflow-hidden px-4
          pt-6 pb-5 sm:px-6 lg:px-8"
      >
            ...
            <div className="mb-3 flex items-center gap-2">
              ...
              <button
                ...
                className="inline-flex shrink-0 items-center justify-center
                  rounded-full border border-[var(--divider)] bg-white p-2
                  text-[var(--subtle)] shadow-sm"
            ...
            {activeFiltersCount > 0 && (
              <button
                ...
                className="inline-flex items-center gap-1 rounded-full px-3 py-1
                  text-[var(--subtle)] transition hover:bg-[var(--tintColor)]
                  hover:text-[var(--text)]"
              >
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  ...
                  >
                    <article className="pi-card h-full p-4">
                      ...
                      >
                        ...
                        <span
                          className="pi-chip px-2 py-1 text-[11px] font-medium"
                        >
                          {t("deviceType")}
                        </span>
                      </div>
              ...
              {filteredDevices.length === 0 && (
                <div
                  className="col-span-full flex min-h-[40vh] flex-col
                    items-center justify-center gap-2 rounded-[1.5rem] border
                    border-dashed border-[var(--divider)] bg-white text-center
                    shadow-sm"
                >
                  <Server className="h-10 w-10 text-[var(--home-ruyi-blue)]" />
                  <div className="text-lg font-medium">{t("noDevices")}</div>
                  <div className="max-w-md text-sm text-[var(--subtle)]">
                    {t("adjustFilters")}
                  </div>
                </div>
              )}
            ...
            <div className="space-y-3">
                    ...
                    >
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span
                            className="rounded-md bg-[var(--tintColor)] px-2
                              py-0.5 font-mono text-xs text-[var(--subtle)]"
                          >
                            {pkg.version}
                          </span>
                        </div>
              ...
              {filteredPackages.length === 0 && (
                <div
                  className="flex min-h-[40vh] flex-col items-center
                    justify-center gap-2 rounded-[1.5rem] border border-dashed
                    border-[var(--divider)] bg-white text-center shadow-sm"
                >
